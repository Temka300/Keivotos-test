...
Keivotos uses a PyInstaller one-folder build. One-folder is intentional: the web assets and separately licensed command-line tools remain visible, replaceable, and easier to inspect than they would be in a single self-extracting binary.
...
GitHub Actions packaging is intentionally deferred. Build and inspect the ZIP and checksum locally before attaching them to a release.
